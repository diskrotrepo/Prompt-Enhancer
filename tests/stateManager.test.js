...
describe('Mix state roundtrip', () => {
  ...
  test('mix dropout applies interleave randomization before deletion', () => {
    ...
    expect(output).toBe('a2 b1 a3 ');
  });

  test('mix dropout applies full-randomize after deletion', () => {
    const output = runGeneratedOutput(
      {
        mixes: [
          {
            type: 'mix',
            title: 'Dropout Full Random',
            limit: 9,
            lengthMode: 'dropout',
            preserve: true,
