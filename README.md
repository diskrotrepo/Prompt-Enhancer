...
The **Completion API** start-menu app is for continuation-oriented requests. Its working surface has two modes: **Autocomplete** continues Beginning text, while **Infill** places a generated middle between Beginning text and Ending text. Provider, model, key, and mode live in one foldable Settings panel; endpoint and sampling controls are nested under Advanced request settings. Models without a documented native FIM contract disable Infill and fall back to Autocomplete. Every adapter sends a top-level `prompt` and never sends a `messages` array. Infill translates Ending text to the provider's documented `suffix` wire field only for DeepSeek FIM, Mistral FIM, or OpenAI's exact model-specific legacy path. Before any credential or prompt leaves the browser, the app requires an absolute HTTP(S) URL ending in `/completions`; Chat Completions, Responses, Messages, relative, and non-HTTP endpoints are rejected.
...
Catalog parsing follows each provider's current native shape rather than assuming OpenAI uniformity. DeepSeek intersects its authenticated account list with the FIM schema's exact `deepseek-v4-pro` ID; DeepInfra reads its authenticated OpenAI-shaped `/v1/models` list, requires the exact nested `metadata.tags` value `text-generation`, applies `metadata.context_length`/`max_tokens`, and converts its published per-million prices to per-token estimates; Fireworks reads its documented account `models` collection and `supportsServerless` flag, then follows every `nextPageToken` because one response is capped at 200 records; Together reads the top-level array, keeps only `language`/`code` entries, and converts catalog prices from per-million to per-token before estimating cost; Mistral requires `capabilities.completion_fim`; OpenAI keeps only the three documented legacy completion IDs; and OpenRouter honors per-model `supported_parameters`, `default_parameters`, and mandatory-reasoning metadata. Hyperbolic uses its one documented sunset base ID without making an undocumented catalog request. A successful catalog with zero compatible models remains empty, while documentation-backed fallbacks are used only when discovery is unavailable.

Request bounds are provider-aware rather than cosmetic slider limits. DeepSeek FIM is capped at 4K output; Together temperature is capped at 1; Mistral FIM temperature is capped at 1.5; and known model output/context limits constrain `max_tokens`. DeepInfra exposes only its raw-route guide fields (`max_tokens`, temperature, `top_p`, and provider-controlled stop), while parameters unsupported by any active provider or selected OpenRouter model are disabled and omitted from JSON; missing OpenRouter `supported_parameters` metadata is treated conservatively as no permission for optional fields. There is no manual stop-sequence control. The client preserves an automatic stop only when an official catalog explicitly supplies a literal default (currently OpenRouter's documented `default_parameters.stop`) and the value fits that provider's array limit; otherwise it omits `stop` and relies on native EOS or `max_tokens`. FIM sentinel/start/middle tokens are likewise never guessed in the browser—the native endpoint's tokenizer adapter owns them.
...
Each provider retains an isolated API key, endpoint, and model while the user switches adapters. Encrypted Completion settings store those maps, the active mode, and Beginning/Ending drafts inside a Completion-specific versioned schema, validate product kind/version and every endpoint before mutating the window, and make a complete restored provider/model/key ready without another catalog request. Version 2 removes persisted manual stops; version 1 files remain loadable and migrate their old FIM suffix into Ending text.

### How native Infill remains autoregressive

Infill does not require diffusion or a bidirectional decoder. During FIM training, a document is split into prefix, middle, and suffix, then serialized with sentinel tokens in an order such as **prefix → suffix → middle**. The same causal next-token model can therefore read both outer contexts to its left and generate the missing middle left-to-right. The [original FIM paper](https://arxiv.org/abs/2207.14255) describes this as a data transformation rather than a new model architecture. This app sends the human-level Beginning and Ending strings; DeepSeek, Mistral, or OpenAI's native adapter performs its own tokenizer-specific serialization.

The documented APIs do not list a separate Infill surcharge. Billing still follows the selected model's input/output token prices: Beginning **plus Ending** consume input context, and the generated middle consumes output tokens. Consequently, Infill can cost more than the same Beginning-only Autocomplete request because Ending text adds input tokens, not because the endpoint changes the per-token rate. Provider pricing and caching rules can still differ and change over time.
...
- **Custom preset ids and unused definitions stay with their own prompt across window opens; custom names may override built-in ids** — `prompt_complete_state_roundtrip`, `tests/storageManager.test.js`
- **Append-save color collisions remap imported references without changing existing colors, including built-in references** — `append_palette_collision_roundtrip`, `tests/storageManager.test.js`
- **Named preset edits update every linked box before saving; imported built-in colors survive same-id custom overrides** — `edited_preset_and_builtin_import_roundtrip`, `tests/storageManager.test.js`
...
- **Every root/nested Mix, String, and Variable retains its type, order, id, and source references through JSON save/load** — `prompt_complete_state_roundtrip`, `tests/storageManager.test.js`
- **Collapsed mix/string/variable UI state and blank editable titles roundtrip through save/load** — `collapsed_state_persisted`, `prompt_complete_state_roundtrip`
- **Preserve chunks retains its dormant numeric/custom size as well as first-chunk behavior** — `prompt_complete_state_roundtrip`, `tests/storageManager.test.js`
- **An explicitly empty prompt reopens empty; empty append saves add no boxes but retain included palette definitions** — `empty_prompt_roundtrip`, `empty_save_imports_palette_only`
...
- **Completion settings collapse to one provider/model/mode summary, with endpoint and sampling controls nested under Advanced; the cool procedural mat reuses prompt-box texture conventions without patterning functional silver fields** — `openrouter_app_window`
- **Autocomplete shows no Ending field, while native Infill orders Beginning text, generated middle, then Ending text and is disabled automatically for unsupported models** — `openrouter_app_window`, `tests/openrouterApp.test.js`
...
- **Completion API treats an explicitly returned empty completion text as a successful blank response** — `tests/openrouterApp.test.js`
- **Completion API copies intentionally blank output without treating it as failure** — `tests/openrouterApp.test.js`
- **OpenRouter, DeepInfra, Fireworks, Together, OpenAI Legacy, and Hyperbolic Autocomplete emit `prompt` with no `messages`; explicitly selected DeepSeek/Mistral Infill adds documented `suffix`, while OpenAI Infill is limited to `gpt-3.5-turbo-instruct`** — `tests/openrouterApp.test.js`
- **There is no manual stop or tokenizer-sentinel field: exact catalog `default_parameters.stop` metadata is preserved within provider limits, malformed defaults are omitted, and native FIM adapters own their special tokens** — `tests/openrouterApp.test.js`
- **Provider/model capability controls omit unsupported fields, treat missing OpenRouter parameter metadata as permission for no optional fields, enforce DeepSeek's 4K cap, Together's temperature-1 cap, Mistral FIM's temperature-1.5 cap, and known model token/context limits; they convert DeepInfra/Together per-million prices correctly and adapt Mistral's message-wrapped FIM response without changing its request shape** — `tests/openrouterApp.test.js`
...
- **Completion encrypted settings validate Completion kind/version and all endpoints before mutation, preserve every provider's key/endpoint/model plus mode and text drafts, migrate version-1 suffix drafts, and restore a complete selection without a redundant catalog request** — `tests/openrouterApp.test.js`
...
- **Color presets**  
  Custom color presets are shared by boxes within one prompt and persist with that prompt, including unused definitions. Opening another window cannot replace the palette. Saved ids remain stable even when labels differ; a user definition can override a built-in name. Missing preset ids fall back to Auto on load.
...
- **Prompt file contents**
  The legacy `mixes` array contains every root box in order: Mix, String, or Variable. Mix children recurse with the same rules. Saves retain ids, references, literal source text, titles (including blanks), length/order/delimiter settings, dormant Preserve/empty-string settings, colors and custom presets, and collapse state for every box type. An empty array means an intentionally empty prompt. Output panels are generated results rather than saved snapshots; Generate recalculates them after Open, so random modes can produce a new result.

- **Add Save append semantics**
  The root and mix action rows include **+ Add Save**. It opens a JSON file picker, reads the saved `mixes` array, and appends those top-level boxes exactly where the button was clicked. Imported ids are re-keyed against the receiving prompt tree when needed, and variables inside the imported save are remapped to the imported copies. Conflicting color-preset ids are remapped independently so both documents keep their colors. A save with no boxes may still import palette definitions.
