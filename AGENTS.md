...
The UI is a Windows 3.1 style desktop (procedural large-shape 90s confetti over a gradually changing textured ground, silver beveled windows, navy active title bars, and restrained per-box procedural pattern mats that expose prompt nesting); see the Theme section in `src/AGENTS.md` before changing styles so new controls reuse the shared `--w31-*` tokens, bevel recipes, and opaque silver work surfaces.
